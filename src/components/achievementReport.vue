...
import { getEverydayList, getEverydayTotal, getYearReportForm } from '@/api/user'
export default {
    data() {
        return {
            pickerOptions: {
    ...
    methods: {
        ...
        getYearReportForm() {
            ...
            getYearReportForm(obj).then((item) => {
                console.log(item)
                this.pieData = []
                this.pieTotalNum = item.data.all_total_price
                item.data.month_list.forEach((item) => {
                    console.log()
                    if (item.date == new Date().getFullYear() + '' + (new Date().getMonth() + 1)) {
